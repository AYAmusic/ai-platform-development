...
@router.get("/comfyui/progress")
async def get_comfyui_progress(request: Request, user=Depends(get_verified_user)):
    """Get real-time progress from ComfyUI"""
    try:
        base_url = request.app.state.config.COMFYUI_BASE_URL
        if not base_url:
            raise HTTPException(status_code=400, detail="ComfyUI base URL not configured")
        ...
        async with aiohttp.ClientSession() as session:
            async with session.get(f"{base_url}/progress", headers=headers) as response:
        ...
        log.error(f"Error getting ComfyUI progress: {e}")
        return {"error": str(e)}

@router.get("/comfyui/queue")
async def get_comfyui_queue(request: Request, user=Depends(get_verified_user)):
    ...
    try:
        ...
        async with aiohttp.ClientSession() as session:
            async with session.get(f"{base_url}/queue", headers=headers) as response:
                if response.status == 200:
        ...
        log.error(f"Error getting ComfyUI queue: {e}")
        return {"error": str(e)}
